<template>
  <div>
    ...
    <!-- 文章等待加载中 -->
    <van-loading color="#0094ff" v-if="Object.keys(artObj).length === 0"> 你这网速得加钱…… </van-loading>
...
export default {
  ...
  methods: {
    formatDate: timeAgo,
    // 关注/取关作者
    // 注意：每个登录的手机号，都维护着自己的关注列表，建议用自己的手机号登录
    ...
    async loveFn (bool) {
      if (bool === true) {
        const res = await unlikingsAPI({
          articleId: this.artObj.art_id
        })
        this.artObj.attitude = 0
        console.log(res)
      } else {
        const res = await likingsAPI({
          articleId: this.artObj.art_id
        })
