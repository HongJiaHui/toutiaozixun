...
<div>
    ...
    <div>
      <!-- van-form整体表单组件@submit 自定义事件（提交事件）-->
      <!-- van-field 输入框（表单一项）
      输入框name属性="用户名" 表单整体提交时，默认收集一个提交对象往后台发送的参数名就是name里的值 {"用户名"："页面值"} -->
      <!-- :属性名="表达式"   属性名="字符串" -->
  ...
  <div style="margin: 16px;">
    <van-button round block type="info" native-type="submit" :disabled="isLoading" :loading="isLoading" loading-text="正在登录……">登录</van-button>
  </div>
</van-form>
    </div>
    </div>
...
//  目标4：点击登录后给用户提示（正在登陆中），并且防止用户频繁的点击
  ...
  methods: {
    ...
    async onSubmit (values) {
      ...
      try {
        ...
        this.$router.replace({
          // 为什么推荐写全？ 因为我们路由规则里/layout里没有重定向，所有直接在这里写全
...
// /deep/会把属性选择器加到前面（用后代选择器的方式往里面找匹配的类名）
//结论：要修改组件内样式，如果你用了scoped，就需要在选择器钱/deep/即可
